...
export const normaliseProperties = (publication, properties, timeZone) => {
  const { categories, channels, me, slugSeparator } = publication;
  ...
  if (properties.category) {
    properties.category = getCategoryProperty(properties, categories);
...
/**
 * Get category property — normalise on write (Category Governance, Layer 1).
 *
 * The single write chokepoint that prevents category fragmentation: trims,
 * drops empties, de-dupes case-insensitively, and folds each authored category
 * to an existing canonical casing when a case-insensitive match exists in the
 * publication's category list. Unknown categories are kept as authored — new
 * categories are allowed; only case-duplicates of existing ones are collapsed.
 * Covers ALL writes (Micropub clients and the admin form, which POSTs here).
 * @param {object} properties - JF2 properties (reads `category`)
 * @param {Array<string>|*} categories - `publication.categories` (canonical
 *   list; a non-array value, e.g. a remote `{url}` config, skips case-folding)
 * @returns {Array<string>} Normalised `category` array
 */
export const getCategoryProperty = (properties, categories) => {
  const authored = Array.isArray(properties.category)
    ? properties.category
    : [properties.category];

  // Case-insensitive lookup: lowercased name → canonical (first-wins) casing.
  ...
  for (const raw of authored) {
    ...
    if (!trimmed) continue;
    const value = canonical.get(trimmed.toLowerCase()) || trimmed;
