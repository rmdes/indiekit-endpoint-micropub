 ...
 * @param {...any} hookArguments - Arguments forwarded to the method
 */
async function callSyndicatorHook(publication, method, ...hookArguments) {
  ...
  await Promise.allSettled(
    targets
      ...
      .map(async (s) => {
        try {
          await s[method](...hookArguments);
        } catch (error) {
          debug(
            `[syndicate:hook] ${method} failed for "${s.name}": ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }),
  );
