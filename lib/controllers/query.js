 ...
 * @property {string} [category] - Only posts with this category
 ...
 * @property {string} [search] - Only posts whose name or content match this text
  ...
  try {
    ...
    switch (q) {
      ...
      case "source": {
        ...
        const categoryParameter = request.query.category;
        const searchParameter = request.query.search;
        const hasExtraFilter = Boolean(categoryParameter || searchParameter);
        ...
        if (postsCollection) {
          if (hasExtraFilter) {
            ...
            if (categoryParameter) {
              filterQuery["properties.category"] = String(categoryParameter);
            }
            if (searchParameter) {
              // Escape regex special characters to prevent injection
              const escaped = String(searchParameter).replaceAll(
                /[.*+?^${}()|[\]\\]/g,
                String.raw`\$&`,
              );
              filterQuery.$or = [
